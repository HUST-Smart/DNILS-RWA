...
#define RESOLVED_CONCAT3(a, b, c, d)  VERBATIM_CONCAT3(a, b, c, d)
	...
	public:
		...
		template<typename Weight = Length, typename NextID = List<List<ID>>>
		static void findAllPairsPaths_asymmetric(Arr2D<Weight> &adjMat, NextID &nextID) {
		...
		template<typename Weight = Length, typename NextID = List<List<ID>> >
		static void findAllPairsPaths_symmetric(Arr2D<Weight> &adjMat, NextID &nextID) {
			ID nodeNum = adjMat.size1();
			...
			for (ID mid = 0; mid < nodeNum; ++mid) {
				...
				for (ID src = 0; src < nodeNum; ++src) {
					...
					for (ID dst = 0; dst < src; ++dst) {
						...
						if (w < srcVec[dst]) {
							nextID[src][dst] = nextID[src][mid];
							srcVec[dst] = adjMat.at(dst, src) = w;
						}
					}
				}
			}
...
/*template<typename Weight = Length>
	static void findAllPairsPaths_symmetric(Arr2D<Weight> &adjMat,List<List<ID>> &nextID) {
		ID nodeNum = adjMat.size1();
